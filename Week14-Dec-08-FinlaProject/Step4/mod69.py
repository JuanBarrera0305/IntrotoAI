"""
CHAPTER 6 + CHAPTER 9 — Finalists from Prelim Results
------------------------------------------------------
...
Columns expected:
    Athlete ID, Name, School, Heat, Lane, SeedTime, SimTime, Place

Rule for finalists:
    - Top 2 per heat (by Place) qualify automatically  -> "Auto"
    - Next 6 fastest remaining (by SimTime, then SeedTime, then Athlete ID) -> "Time"
    - Total finalists = 32

Chapter 6 (CSP):
    - Variables: Qualify[athlete_id] ∈ {0, 1}
    - Constraints:
        * Exactly top-2 in each heat must have Qualify = 1 (Auto).
        * Exactly 6 additional athletes with best remaining times must have Qualify = 1 (Time).
        * Total qualified = 32.

Chapter 9 (Inference with Horn clauses):
    - Ground facts derived from the CSP solution:
        CSPAuto(a), CSPTime(a)
    - Rules:
        1) CSPAuto(a) → Auto(a)
        2) CSPTime(a) → TimeQual(a)
        3) Auto(a)    → Qualifies(a)
           TimeQual(a) → Qualifies(a)
    - Plus auxiliary facts about Athlete, Heat, Place, SimTime to keep it “real” AI.

    - We then check that:
        * {a | Auto(a)} == CSP Auto set
        * {a | TimeQual(a)} == CSP Time set

Output:
    Semifinals_Qualifiers_from_sim.csv
"""
